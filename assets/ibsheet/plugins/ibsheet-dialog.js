...
Fn.initImportSheet = function (uploadType, width, height, name, colCount, fullLoadData) {
  ...
  var defaultEvents = {
    onRenderFirstFinish: function (evtParams) {
      ...
      if (evtParams.sheet.RowIndex) sheetCols = sheetCols.filter(function(item) { return item !== "SEQ"; });
      var colsMap = "";
      ...
      if (uploadType === "excel") {
        evtParams.sheet.loadExcel({mode: "NoHeader", columnMapping: colsMap, mapSEQ: true});
      }
      else if (uploadType === "txt" || uploadType === "csv") {
        evtParams.sheet.loadText({mode: "NoHeader", colSeparator: " !  @  ", fileExt: uploadType === "csv" ? "csv" : "", mapSEQ: true});
      }
    },
